...
class CheckBox:
    def __init__(self, rect , text, font = constant.FONT, visible = True, clicked = True, color = constant.COLOR_INACTIVE):
        ...
        self.checkobject = ButtonSwitching(pg.Rect(self.rect.x + 5,self.rect.y + 5,23,23))
        if self.clicked:
    ...
    def draw(self, screen):
        if self.visible:
            screen.blit(self.font.render(self.text, True, self.color), (self.rect.x+35, self.rect.y+5))
...
if __name__ == '__main__':
    ...
    checkbox = CheckBox(pg.Rect(10, 300,64,32), "checked")
